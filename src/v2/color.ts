...
function Collor() {
  const MIN = 0
  const MAX = 255
  const serialize = NumberSerializer(MIN, MAX)

  function createByDecimal(value: number) {
    const decimal = serialize(value)
  ...
  function createByHex(value: string) {
    if (value.length !== 1 && value.length !== 2) {
      throw new Error('invalid code')
    }
    if (value.length === 1) {
      return createByDecimal(parseInt(`${value}${value}`, 16))
    }
    return createByDecimal(parseInt(value, 16))
  }
...
export class Color {
  ...
  private static readonly serialize = NumberSerializer(this.MIN, this.MAX)
  ...
  static fromDecimal(value: number) {
    const decimal = Color.serialize(value)
    const hex = decimal.toString(16).padStart(2, '0')
